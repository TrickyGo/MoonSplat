...
from tracking.retrieval_database import RetrievalDatabase
import tracking.matching as matching

DATASET_IMG_DOWNSAMPLE = 1
DATASET_IMG_SIZE = 512

def load_mast3r(path=None, device="cuda"):
    weights_path = (
        "/data/pug/MASt3R-SLAM/checkpoints/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth"
        if path is None
        else path
    )
...
def load_retriever(mast3r_model, retriever_path=None, device="cuda"):
    retriever_path = (
        "/data/pug/MASt3R-SLAM/checkpoints/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric_retrieval_trainingfree.pth"
